...
class Chat:
    def __init__(self, cfg, model, vis_processor, device='cuda:0'):
        self.device = device
        self.model = model
        self.vis_processor = vis_processor
        self.image_vis_processor = Blip2ImageEvalProcessor()
        # quant_config_8bit = BitsAndBytesConfig(
        #     load_in_8bit=True,               # chuyển về int8
        #     llm_int8_threshold=6.0,          # ngưỡng chọn int8 vs fp16 (tùy model)
        # )
        # self.generation_model = AutoModelForCausalLM.from_pretrained(
        #     cfg.llama_model,    # <— your local path
        #     # quantization_config=quant_config_8bit,
        #     device_map="auto"
        # ).cuda()
        # self.generation_tokenizer = AutoTokenizer.from_pretrained(
        #     cfg.llama_model, use_fast=False
        # )
